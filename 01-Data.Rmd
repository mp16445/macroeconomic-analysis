...
We will assume that you know what GDP represents, how it is constructed and its limitations. We will use data from the <a href="https://www.rug.nl/ggdc/historicaldevelopment/maddison/releases/maddison-project-database-2020?lang=en" target="_blank">*Maddison Project Database*</a> [@mad1] and the <a href="https://www.rug.nl/ggdc/productivity/pwt/" target="_blank">*Penn World Table*</a> [@PWT], which allow us to compare *real* economic outcomes over time and across countries. GDP in this chapter will always refer to real GDP although this may not be explicitly mentioned for simplicity in the text.
...
```{r mpdplot1, echo=FALSE, fig.align='center', fig.cap= "Long-run trends in GDP per capita: 1700-2018", warning=FALSE}

rm(list=ls())
mpd2020 <- read_csv("Data/mpd2020.csv",show_col_types = FALSE) %>% select(country, year, gdppc) %>% filter(country == "United Kingdom" | country == "United States" | country == "China" | country == "Nigeria" | country == "Japan" | country == "Brazil" | country == "South Africa" | country == "Indonesia" | country == "India" | country == "France" | country == "Germany" | country == "Russian Federation" | country == "Turkey" | country == "Italy" | country == "Mexico", year >= 1700) 
dates <- seq(from = 1700, to = 2018, by = 30)

mpd2020 %>% ggplot(aes(x = year, y = gdppc, colour = country)) + geom_line()+ labs(x = "", y = "GDP per capita (in 2011$)", caption = "Source: Maddison Project Database (MPD) 2020") + scale_x_continuous(breaks = dates)+ scale_y_continuous(limits = c(0,60000), breaks = c(0,10000,20000,30000,40000,50000,60000)) + theme_bw() + theme_minimal() + theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(),panel.grid.major.y = element_line(),plot.title = element_text(hjust = 0.5), legend.title=element_blank()) 
#,label = c(0,2,4,8,16,32,64,128)
```
...
rm(list=ls())
mpd2020 <- read_csv("Data/mpd2020.csv",show_col_types = FALSE) %>% select(country, year, gdppc) %>% 
  filter(country =="United States", year >= 2000 & year < 2019) %>% 
  mutate(gdpgr = (gdppc / lag(gdppc) - 1)*100) %>% 
  select(year, gdppc, gdpgr)

kbl(mpd2020, align = "ccc",col.names = c('Year (t)','GDP per capita (in $2011)', 'Growth rate (%)'), caption="US GDP per capita and growth rate (2000-2018)", digits = 2) %>%
  kable_styling(bootstrap_options = c("striped", "hover", "condensed"), position = "center") %>%
  kableExtra::footnote(general = "Source: Maddison Project Database 2020.", general_title = "")

```

```{r DiscreteTab_b, eval=FALSE, echo=FALSE, warning=FALSE}

rm(list=ls())
mpd2020 <- read_csv("Data/mpd2020.csv",show_col_types = FALSE) %>% select(country, year, gdppc) %>% 
  filter(country =="United States", year >= 2000 & year < 2005) %>%
  select(year, gdppc)

table1 <- flextable(mpd2020) %>% colformat_double(j = 2, digits = 2) %>%
  add_footer_lines("Source: Maddison Project Database (MPD) 2020") %>%
  set_caption("Table 1") %>%
  set_header_labels(values = list(year = "Year(t)", gdppc = "US real GDP per capita (in $2011)")) %>%
...
```{r mpdplot3, echo=FALSE, fig.align='center', fig.cap= "GDP per capita in the United States: 1870-2018", warning=TRUE}

rm(list=ls())
mpd2020 <- read_csv("Data/mpd2020.csv",show_col_types = FALSE) %>% select(country, year, gdppc) %>% filter(country == "United States", year >= 1870) %>% 
  mutate(gdpgr = (gdppc / lag(gdppc) - 1)*100)
dates <- seq(from = 1870, to = 2018, by = 20)

#average <- mean(mpd2020c$gdpgr, na.rm = TRUE)

mpd2020 %>% ggplot(aes(x = year, y = gdppc)) + geom_line(colour = "black", linewidth = 1) + stat_smooth(method=nls, method.args = list(start = c(a=4803, b=0.015)),formula = y ~ a*(1 + b)^(x-1870), colour = 'red', linewidth = 0.75, linetype = 2, se = FALSE) + labs(x = "", y = "GDP per capita (in 2011$)", caption = "Source: Maddison Project Database (MPD) 2020") + scale_x_continuous(breaks = dates)+ scale_y_continuous(limits = c(0,60000), breaks = c(0,10000,20000,30000,40000,50000,60000)) + theme_bw() + theme_minimal() + theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(),panel.grid.major.y = element_line(),plot.title = element_text(hjust = 0.5), legend.title=element_blank()) 

#stat_smooth(method=nls, method.args = list(start = c(a=4803, b=0.015)),formula = y ~ a*exp(b*(x-1870)), colour = 'red', size = 0.75, linetype = 2, se = FALSE)

#The trend growth rate is not the **average growth rate**, the average of the annual growth rates calculated for each year, which is equal to 1.77% in this example. The average growth rate also takes into account compounding effects but uses more information and therefore is computationally more intensive, being calculated using 149 observations instead of 2. In addition, it is affected by short-term fluctuations such as the large swings in GDP per capita observed around the Great Depression and the second-world war, which we want to ignore when describing long-run trends. 
...
**Example: GDP per capita in the United States (1870-2018)**
...
```{r mpdplot4, echo=FALSE, fig.align='center', fig.cap= "(Level and logarithm of) GDP per capita in the United States: 1870-2018", warning=FALSE, message=FALSE, fig.width=10, fig.height=3}
...
mpd2020 <- read_csv("Data/mpd2020.csv",show_col_types = FALSE) %>% 
  ...
  rename(t = "year") 
dates <- seq(from = 1870, to = 2018, by = 40)
...
plot2 <- mpd2020 %>% ggplot(aes(x = t, y = lny)) + geom_line(colour = "red") + labs(x = "", y = "(ln) GDP per capita") + scale_x_continuous(breaks = dates) + theme_bw() + theme_minimal() + theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(),panel.grid.major.y = element_line(),plot.title = element_text(hjust = 0.5))

plot3 <- mpd2020 %>% ggplot(aes(x = t, y = lny)) + geom_line(colour = "red") + geom_smooth(method=lm,se=FALSE) + labs(x = "", y = "(ln) GDP per capita", caption = "Source: Maddison Project Database (MPD) 2020") + scale_x_continuous(breaks = dates) + theme_bw() + theme_minimal() + theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(),panel.grid.major.y = element_line(),plot.title = element_text(hjust = 0.5))
...
lmtrend <- lm(lny ~ t,mpd2020c)
equatiomatic::extract_eq(lmtrend, use_coefs = TRUE, coef_digits = 4)
```
...
```{r mpdplot2, echo=FALSE, fig.align='center', fig.cap= "GDP per capita in the United States and United Kingdom: 1800-2018", warning=FALSE,fig.width=8, fig.height=4}

rm(list=ls())
mpd2020 <- read_csv("Data/mpd2020.csv",show_col_types = FALSE) %>% select(country, year, gdppc) %>% filter(country == "United Kingdom" | country == "United States", year >= 1800) 
dates <- seq(from = 1800, to = 2018, by = 100)

mpd2020 %>% ggplot(aes(x = year, y = gdppc, colour = country)) + geom_line()+ labs(x = "", y = "GDP per capita (in 2011$)", caption = "Source: Maddison Project Database (MPD) 2020") + scale_x_continuous(breaks = dates)+ scale_y_continuous(limits = c(0,60000), breaks = c(0,10000,20000,30000,40000,50000,60000)) + theme_bw() + theme_minimal() + theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(),panel.grid.major.y = element_line(),plot.title = element_text(hjust = 0.5), legend.title=element_blank()) 

#Figure \@ref(fig:mpdplot2) represents the same data for the United States and the United Kingdom since 1500. By going back five centuries for the United Kingdom, it serves to highlight how unprecedented the modern growth experience has been over the very long-run.  
```
...
We can address these questions using the techniques developed in the previous sections. Figure \@ref(fig:mpdplot6) plots the logarithm of GDP per capita for both countries and can be used to identify subperiods. Table \@ref(tab:mpd2020Tab) presents estimates of the trend growth rates of GDP per capita for both the United States and the United Kingdom for the full sample period and four subperiods.

```{r mpdplot6b, echo=FALSE, fig.align='center',fig.cap= "GDP per capita: United States vs United Kingdom (1800-2018)",fig.width=10, fig.height=3, eval=FALSE, message=FALSE}

rm(list=ls())
mpd2020 <- read_csv("Data/mpd2020.csv") %>% 
  select(country, year, gdppc) %>% 
  filter(country == "United Kingdom" | country == "United States", year >= 1800) %>% 
  mutate(lny = log(gdppc))
dates <- seq(from = 1800, to = 2018, by = 40)

plot1 <- mpd2020 %>% ggplot(aes(x = year, y = lny, colour = country)) + geom_line()+ labs(x = "", y = "", caption = "Source: Maddison Project Database (MPD) 2020") + scale_x_continuous(breaks = dates) + theme_bw() + theme_minimal() + theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(),panel.grid.major.y = element_blank(),plot.title = element_text(hjust = 0.5), legend.title=element_blank()) + labs(title = "(ln) GDP per capita")

mpd2020 <- mpd2020 %>% select(country, year, gdppc) %>% 
...
plot2 <- mpd2020 %>% ggplot(aes(x = year, y = ratio, colour = country)) + geom_line()+ labs(x = "", y = "", caption = "Source: Maddison Project Database (MPD) 2020") + scale_x_continuous(breaks = dates) + theme_bw() + theme_minimal() + theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(),panel.grid.major.y = element_line(),plot.title = element_text(hjust = 0.5), legend.title=element_blank()) + labs(title = "GDP per capita relative to UK (UK=1)")

plot1 + plot2
```

```{r mpdplot6, echo=FALSE, fig.align='center',fig.cap= "United States vs United Kingdom: GDP per capita (1800-2018)",fig.width=6, fig.height=3.5, eval=TRUE, message=TRUE}
...
#mpd2020 %>% ggplot(aes(x = year, y = lny, colour = country)) + geom_line()+ labs(x = "", y = "", caption = "Source: Maddison Project Database (MPD) 2020") + scale_x_continuous(breaks = dates) + theme_bw() + theme_minimal() + theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(),panel.grid.major.y = element_blank(),plot.title = element_text(hjust = 0.5), legend.title=element_blank()) + labs(title = "Natural logarithm of GDP per capita")
...
Over the period *1946-2018*, the trends in GDP per capita are parallel in the two countries, suggesting that GDP per capita grew at relatively similar rates. However, between *1800* and *1946*, GDP per capita grew at a substantially higher trend growth rate in the United States, which is the period when US GDP per capita overtook UK GDP per capita. 

```{r mpd2020Tab, echo=FALSE, warning=FALSE, eval=TRUE, message=TRUE}
...
break_dates <- c(1800,1830,1946,2018)
break_nb <- length(break_dates)
date_nb <- length(mpd2020$year)
gy_US <- ((mpd2020$US[date_nb]/mpd2020$US[1])^(1/(break_dates[break_nb]-break_dates[1]))-1)*100
gy_UK <- ((mpd2020$UK[date_nb]/mpd2020$UK[1])^(1/(break_dates[break_nb]-break_dates[1]))-1)*100
table <- as_tibble_row(c("US" = gy_US, "UK" = gy_UK))
index <- c()
for (date in break_dates){
  row_nb <- which(grepl(date,mpd2020$year))
  index <- c(index,row_nb)
}
for (i in seq(1:(break_nb-1))){
  gy_US <- ((mpd2020$US[index[i+1]]/mpd2020$US[index[i]])^(1/(break_dates[i+1]-break_dates[i]))-1)*100
  gy_UK <- ((mpd2020$UK[index[i+1]]/mpd2020$UK[index[i]])^(1/(break_dates[i+1]-break_dates[i]))-1)*100
  table <- add_row(table,US = gy_US, UK = gy_UK)
}
subperiods <- as_tibble_col(c("1800-2018","1800-1830","1830-1946","1946-2018"))
...
kbl(table, align = "ccc",col.names = c("Period","United states", "United Kingdom"), caption="UK and US: Trend growth rates over different subperiods", digits = 2) %>%
  kable_styling(bootstrap_options = c("striped", "hover", "condensed"), position = "center") %>%
  kableExtra::footnote(general = "Source: Maddison Project Database 2020. Compound annual growth rates (%).", general_title = "")

```


```{r mpd2020Tab_b, echo=FALSE, warning=FALSE, eval=FALSE, message=TRUE}
...
break_dates <- c(1800,1830,1878,1946,2018)
break_nb <- length(break_dates)
date_nb <- length(mpd2020$year)
gy_US <- ((mpd2020$US[date_nb]/mpd2020$US[1])^(1/(break_dates[break_nb]-break_dates[1]))-1)*100
gy_UK <- ((mpd2020$UK[date_nb]/mpd2020$UK[1])^(1/(break_dates[break_nb]-break_dates[1]))-1)*100
...
subperiods <- as_tibble_col(c("1800-2018","1800-1830","1830-1878","1878-1946","1946-2018"))
table <- add_column(subperiods,table)
...
```{r mpd2020Tab_c, echo=FALSE, warning=FALSE, eval=FALSE}
...
break_dates <- c(1800,1830,1870,1945,2018)
break_nb <- length(break_dates)
date_nb <- length(mpd2020$year)
gy_US <- ((mpd2020$US[date_nb]/mpd2020$US[1])^(1/(break_dates[break_nb]-break_dates[1]))-1)*100
gy_UK <- ((mpd2020$UK[date_nb]/mpd2020$UK[1])^(1/(break_dates[break_nb]-break_dates[1]))-1)*100
...
subperiods <- as_tibble_col(c("1800-2018","1800-1830","1830-1870","1870-1945","1945-2018"))
table <- add_column(subperiods,table)

table3 <- flextable(table) %>% colformat_double(j = 2:3, digits = 2) %>% 
  add_footer_lines("Source: Maddison Project Database (MPD) 2020") %>%
  set_caption("Table 3: Average annual growth rates (%)") %>%
  set_header_labels(values = list(value = "Period", US = "United states", UK = "United Kingdom")) %>%
  fontsize(size = 7, part = "footer") %>% 
  align(align = "center", part = "all")
table3
rm(list=ls())
...
dygraph(mpd2020, main = "GDP per capita in Indonesia: 1950-2018") %>% 
...
<br>
Based on the plot above (*Source*: Maddison Project Database) the annual trend growth rate in GDP per capita in Indonesia between 1950 and 2018 was:
...
dygraph(mpd2020, main = "(Logarithm) GDP per capita in Indonesia: 1950-2018") %>% 
...
<br>
The figure above represents the logarithm of GDP per capita in Indonesia (*Source*: Maddison Project Database). Discuss the advantages and disadvantages associated with each of these subsamplings i) if the goal were to analyse long-run growth trends in GDP growth in Indonesia and ii) if you goal were to systematically compare GDP growth in different countries over time:

a. 1950-1970, 1970-1990, 1990-2010, 2010-2018
a. 1950-1967, 1967-1997, 1997-2018
a. 1950-1967, 1967-1974, 1974-1989, 1989-1997, 1990-2018
a. 1950-1967, 1967-2018


**Exercise 6**

```{r mpdplot8c, echo=FALSE, fig.align='center', fig.cap= "GDP per capita in Indonesia (ratio scale): 1950-2018",fig.width=6, fig.height=4, warning=FALSE, message=FALSE}
...
plot3 <- mpd2020 %>% ggplot(aes(x = t, y = gdppc)) + geom_line(colour = "blue")+ labs(x = "", y = "GDP per capita (in 2011$)")+ scale_x_continuous(breaks = dates) + scale_y_continuous(trans = 'log2', limits = c(1000,16000), breaks = c(1000,2000,4000,8000,16000)) + theme_bw() + theme_minimal() + theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(),panel.grid.major.y = element_line(),plot.title = element_text(hjust = 0.5))

plot3

```

The figure above represents GDP per capita in Indonesia on a ratio scale (*Source*: Maddison Project Database). 
