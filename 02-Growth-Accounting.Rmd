...
## Exercise lecture (Week 13)

This exercise guides  you through the growth accounting decomposition of UK labour productivity as seen in the lecture slides this week, then ask you to do the same for US labour productivity and to finally compare the results for the two countries.

In order to perform these practice questions, you need to work on a laptop (if possible using a mouse), **not** a tablet. You also need to have installed Excel on your device (you may not be able to perform all the operations using the web version).

### Before the exercise lecture{-}

1. Download and learn more about the *Penn World Table* database by checking the information on this website: [<span style="color: blue;">**Penn World Tables**</span>](https://www.rug.nl/ggdc/productivity/pwt). [*You may also look up this prompt in Copilot: "Explain what the Penn World Table data is, what is its significance, who is responsible for it and what are its objectives".*]
...
### Part 1: Describing long-run trends in productivity {-}
...
### Part 2: Growth accounting decompositions {-}
...
In Excel, construct a table as in the [<span style="color: blue;">**Excel tutorial - Section 7.4**</span>](https://mp16445.github.io/EXCEL_PWT/growth-accounting.html#results-average-decompositions)  containing the full sample period and each subperiod identified in Part 1 in each row.
...
1. Construct a table (as below) and a column chart to summarize the results of the UK labour productivity growth accounting over the full sample period and each subperiod.

1. Repeat 1 to 5 for the US using the same subperiods. Compare the growth accounting results in the two countries.

1. Repeat 1 to 6 using the average of the "labsh" variable to obtain an estimate of the elasticity of capital $\beta$. Under what assumption is this a valid method of estimation for $\beta$? How do the results change when using this alternative measure of $\beta$?

1. (Optional) Repeat 1 to 6 using by using the logarithmic compound growth rate formula $g_{x,t-s}=\frac{1}{s-t} (\ln{x_s}-\ln{x_t})$. How do the results change when using this alternative growth rate formula?

1. (Optional) Repeat 1 to 6 using "rkna" variable to measure capital. How does this measure differ from “rnna”? How would you expect the results to change using this measure of capital? How do the results change when using this alternative measure of the growth of capital?
...
```{r EL - DELETED PART, eval=FALSE, include=FALSE}


### Part 3: Sources of changes in labour productivity {-}

*This will be covered in the lecture if time permits.*
...
```{r Formative exercise, eval=FALSE, include=FALSE}
